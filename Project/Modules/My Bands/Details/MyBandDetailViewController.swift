...
protocol MyBandDetailProviderDelegate: class {
    var delegateDetails: MyBandDetailViewController? { get set }
    func myBandWillAdd(_ controller: MyBandDetailViewController)
}
...
class MyBandDetailViewController: UIViewController {
    ...
    func configView () {
        ...
        genreTextField.addTarget(self, action: #selector(nameDidChange), for: .editingChanged)
        locationTextField.addTarget(self, action: #selector(nameDidChange), for: .editingChanged)
